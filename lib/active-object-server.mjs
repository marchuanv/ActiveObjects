                            ...
                            callback: async () => {
                                ...
                                server.on("request", (request, response) => {
                                    ...
                                    request.on('end', async () => {
                                        ...
                                        requests.unshift({
                                            username, path, hashedpassphrase, sessionauthtoken, content, complete: ({ statusCode, statusMessage, content, contentType }) => {
            ...
            const id = setInterval(async () => {
                try {
                    ...
                    if (requests.length === 0 && state.isRequest === false) {
                        ...
                        clearInterval(id);
                        await server.close();
                        await this.log(`server was stoppped on port ${port}`);
                        await server.listen(port, async () => {
                            state.isStarting = false;
                            await this.log(`server is listening on port ${port}`);
                        });
                        resolve();
                    }
                } catch (error) {
